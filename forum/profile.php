...
if ( !empty($_GET['id']) && valid_int($_GET['id']) ) {
	...
	if ( !$functions->get_config('guests_can_view_profiles') && $session->sess_info['user_id'] == LEVEL_GUEST ) {
		...
		$template->parse('menu', 'panel', array(
			...
			'view_profile' => '<a href="http://www.lockframe.net/profile">'.$lang['ViewProfile'].'</a>',
			'panel_profile' => '<a href="http://www.lockframe.net/profile.php?err=&success=&action=edit">' . ( ( $_GET['act'] != 'editprofile' ) ? $lang['EditProfile'] : '<strong>'.$lang['EditProfile'].'</strong>' ) . '</a>'
		));
